...
export default function NameServerSetup() {
    ...
    const { deployContract, joinContract, api, isLoading, error } = useMidnightContract();
    ...
    const handleDeployRegistry = async () => {
        try {
            addLog("Preparing WNS Deployment Package...");
            // In a real app, WhisperRegistry would be imported from managed
            // We'll mock the contract instance for the UI flow
            const mockContractInstance = {
                // This would normally be the compiled Compact contract
            };

            addLog("Broadcasting WNS Contract to Midnight Network...");
            const newApi = await deployContract(mockContractInstance);
            setContractAddr(newApi.deployedContractAddress);
            addLog(`WNS Deployed Successfully! Address: ${newApi.deployedContractAddress}`);
        } catch (err: any) {
                    ...
                    <div className="space-y-6">
                        <div className="flex gap-4">
                            <div className="flex-1 relative">
                                ...
                                />
                                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] text-[#a692c8] font-bold">.whisper.night</span>
                            </div>
                            <button
